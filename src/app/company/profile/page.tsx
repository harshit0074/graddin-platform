...
export default function StartupProfilePage() {
  const { user, role, isLoading, internships, updateCompanyProfile } = useAuth();
  const { toast } = useToast();

  const company = user?.company;
  const activeInternships = company 
    ? internships.filter(i => (i.company_id === user?.id || i.company_id === company.id) && i.is_active)
    : [];
  ...
  if (isLoading || !company) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        <h2 className="font-serif text-3xl font-bold text-[#1C140E]">Startup Profile</h2>
        <p className="text-sm text-[#72635A] mt-2 mb-6">Please log in with your startup account to view or edit your company profile.</p>
        <Link href="/login">
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      {/* Top Banner & Header (LinkedIn Company Page + Modern Startup Hybrid) */}
