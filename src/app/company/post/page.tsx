...
export default function PostOpportunityPage() {
  ...
  const { user, role, isLoading, postInternship } = useAuth();
  const { toast } = useToast();

  React.useEffect(() => {
    if (!isLoading && role !== 'company' && role !== 'admin') {
      router.push('/login');
    }
...
• Excitement to work in a fast-paced early stage startup environment`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const targetCompanyId = user?.company?.id || (role === 'company' ? user?.id : null);
    if (!targetCompanyId) {
      toast({
        title: "Company profile required",
        description: "Please complete your startup profile or log in to post an opportunity.",
        variant: "destructive"
      });
      return;
    }
    ...
    if (user?.company && !user.company.is_verified) {
      toast({
        ...
        description: "Your startup profile is pending admin verification. You will be able to publish live opportunities once verified.",
        variant: "destructive"
      });
      return;
    }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {user?.company && !user.company.is_verified && (
        <div className="rounded-2xl border border-amber-300 bg-amber-50 p-4 sm:p-5 flex items-start gap-3.5 mb-6">
          <div className="rounded-xl bg-amber-100 p-2 text-amber-800 shrink-0">
            <AlertTriangle className="h-5 w-5" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-[#1C140E]">Startup Verification Under Review</h3>
            <p className="text-xs text-[#72635A] mt-0.5">
              Your startup profile is awaiting verification by Graddin platform administrators. Opportunities can only be published to students once your startup is verified.
            </p>
          </div>
        </div>
      )}
