...
export default function StartupDashboardPage() {
  const { user, internships, applications, updateApplicationStatus, companies } = useAuth();
  
  const currentCompany = user?.company || companies[0];
  const companyInternships = internships.filter(i => i.company_id === currentCompany.id);
  const companyApplications = applications.filter(a => 
    companyInternships.some(i => i.id === a.internship_id) || a.internship?.company_id === currentCompany.id
  );
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-amber-800 mt-0.5 max-w-2xl leading-relaxed">
                We are validating your official LinkedIn company presence ({currentCompany.linkedin_url || 'provided at signup'}). Our admins verify companies within 24 hours to protect students from ghost jobs.
              </p>
            </div>
          </div>
          <Link href="/admin">
            <Button variant="outline" size="sm" className="border-amber-400 bg-white text-xs font-semibold shrink-0">
              <span>Admin Preview (Instant Verify)</span>
