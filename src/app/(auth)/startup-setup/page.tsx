...
export default function StartupSetupPage() {
  ...
  const [formData, setFormData] = useState({
    companyName: user?.company?.company_name || "Velo AI",
    email: user?.company?.email || "founders@velo.ai",
    linkedinUrl: user?.company?.linkedin_url || "https://linkedin.com/company/velo-ai",
    websiteUrl: user?.company?.website_url || "https://velo.ai",
    logoUrl: user?.company?.logo_url || "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=128&auto=format&fit=crop&q=80",
    industry: user?.company?.industry || "Developer Tools & AI",
    location: user?.company?.location || "San Francisco, CA & Remote",
    companySize: user?.company?.company_size || "12-25 people",
    foundedYear: user?.company?.founded_year || "2024",
    about: user?.company?.about || "Velo is building autonomous AI developer agents that convert production incident logs into verified bug fixes and automated integration pull requests.",
    mission: user?.company?.mission || "To eliminate debugging toil and free engineers to focus exclusively on foundational architecture.",
  });
