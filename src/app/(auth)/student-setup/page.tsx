...
export default function StudentSetupPage() {
  ...
  const [formData, setFormData] = useState({
    fullName: user?.profile?.full_name || "Aarav Sharma",
    email: user?.profile?.email || "aarav.sharma@campus.edu",
    avatarUrl: user?.profile?.avatar_url || "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=240&auto=format&fit=crop&q=80",
    headline: user?.profile?.headline || "Aspiring Product Engineer & UI Craftsman",
    university: user?.profile?.university || "BITS Pilani",
    degree: user?.profile?.degree || "B.E. Computer Science & Engineering",
    graduationYear: user?.profile?.graduation_year || "2026",
    location: user?.profile?.location || "Bangalore / Remote",
    skills: user?.profile?.skills || "React, Next.js, TypeScript, Tailwind CSS, Figma, UI/UX Design, Node.js",
    interests: "AI Developer Tools, Micro-Interactions, Distributed Systems, Design Systems",
    experience: user?.profile?.experience || "Built an open-source collaborative code review tool (300+ GitHub stars). Led frontend engineering for campus tech fest web platform serving 25k users.",
    bio: user?.profile?.bio || "Passionate about building fast, intuitive software interfaces for engineering teams.",
    portfolioUrl: user?.profile?.portfolio_url || "https://aaravsharma.design",
    githubUrl: user?.profile?.github_url || "https://github.com/aaravsharma",
    linkedinUrl: user?.profile?.linkedin_url || "https://linkedin.com/in/aarav-sharma-dev",
    resumeText: user?.profile?.resume_text || "Pre-final year student with strong hands-on product engineering skills in Next.js 15, React 19, and full-stack web development.",
  });
