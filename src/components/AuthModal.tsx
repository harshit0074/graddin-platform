...
import { GraduationCap, Building2, ArrowRight, KeyRound } from 'lucide-react';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent onClose={() => onOpenChange(false)} className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-center gap-1 text-xs text-[#72635A] pt-1">
            <span>Prefer password login?</span>
            <Link 
              href="/login" 
              onClick={() => onOpenChange(false)}
              className="font-semibold text-[#2C1B14] hover:underline"
            >
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Link href="/signup" onClick={() => onOpenChange(false)} className="w-full">
              <Button
                ...
                variant="outline"
                size="sm"
                className="w-full text-xs border-[#DFD5C6] bg-white hover:bg-[#F5EFEB]"
              >
